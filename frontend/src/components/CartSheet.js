    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto p-5 space-y-4">
        {cartItems.map(item => {
          ...
          return (
          <div key={key} data-testid={`cart-item-${key}`} className="flex gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-sm text-white/50">&#8377;{unitPrice.toLocaleString('en-IN')}</p>
              <div className="flex items-center gap-2 mt-1.5">
                <button data-testid={`cart-decrease-${key}`} onClick={() => updateQuantity(item, item.quantity - 1)} className="w-6 h-6 rounded border border-white/10 flex items-center justify-center hover:bg-white/5 text-white/60"><Minus size={10} /></button>
                <span className="text-xs w-5 text-center">{item.quantity}</span>
                <button data-testid={`cart-increase-${key}`} onClick={() => updateQuantity(item, item.quantity + 1)} className="w-6 h-6 rounded border border-white/10 flex items-center justify-center hover:bg-white/5 text-white/60"><Plus size={10} /></button>
                <button data-testid={`cart-remove-${key}`} onClick={() => removeFromCart(item)} className="ml-auto text-white/30 hover:text-red-400"><Trash size={14} /></button>
              </div>
            </div>
          </div>
          );
